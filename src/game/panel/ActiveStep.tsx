...
export function ActiveStep({ label, body, button }: ActiveStepProps) {
  return (
    ...
    <div className="aqua-zone-active flex flex-none flex-col gap-3 border-t border-gray-100 px-4 py-3 text-sm">
      <div className="text-xs font-semibold uppercase tracking-[0.03em] text-[#7c8492]">{label}</div>
      {body}
      {button}
    </div>
  );
