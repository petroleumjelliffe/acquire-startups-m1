...
describe('snapshot store', () => {
  it('rewinds to the exact state before a step ran', () => {
    ...
    const stepId = after.log[after.log.length - 1]!.stepId;
    expect(JSON.stringify(rewindTo(store, stepId))).toBe(JSON.stringify(start));
  });

  it('is idempotent — rewinding twice to the same step gives the same state', () => {
    const store = createSnapshotStore();
    const after = applyIntentWithHistory(store, twoPlayers(), { type: 'placeTile', playerId: 'p1', coord: 'A1' });
    const stepId = after.log[after.log.length - 1]!.stepId;
  ...
  it('drops forward entries, so a rewound store cannot resurrect the future', () => {
    const store = createSnapshotStore();
    let s = twoPlayers();
    s = applyIntentWithHistory(store, s, { type: 'placeTile', playerId: 'p1', coord: 'A1' });
    const stepId = s.log[s.log.length - 1]!.stepId;
  ...
  it('rewind-then-replay reaches the same state as an uninterrupted run', () => {
    ...
    const store = createSnapshotStore();
    let s = twoPlayers();
    s = applyIntentWithHistory(store, s, intents[0]);
    const stepId = s.log[s.log.length - 1]!.stepId;
...
/** The state before `stepId` ran. Every entry from `stepId` forward is dropped. */
